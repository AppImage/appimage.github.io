...
for INPUTBASENAME in database/*; do
  ...
  if [ -f database/$INPUTBASENAME/*appdata.xml ] ; then
    appstreamcli.AppDir/root_overlay/lib/x86_64-linux-gnu/ld-2.23.so --inhibit-cache --library-path squashfs-root/root_overlay/usr/lib/x86_64-linux-gnu/ appstreamcli.AppDir/root_overlay/usr/bin/appstreamcli database/$INPUTBASENAME/*appdata.xml database/$INPUTBASENAME/appdata.yaml
